# [SUPERSEDED] Agent Version Lifecycle on UK8s — Controller-Watch Design (v1)

> **This document describes a design that was considered and then
> superseded.** It is kept here for the record and because the reasoning
> for rejecting it (§3 below, plus the new §"Why this was superseded") is
> itself useful context. The current recommended design is the
> lifecycle-hook approach in `ARCHITECTURE.md` — read that one first.

**Why this was superseded:** this design still requires a standalone
controller process (no CRD, but still a custom Deployment watching other
Deployments via label selector) to exist and stay available at all times.
Given the newer, stricter requirements — always available, consistent, and
minimal deployment latency — an externally-watching controller is a
structurally slower and less resilient mechanism than folding the same
logic into native Kubernetes pod lifecycle hooks (`postStart`/`preStop`),
which require no separate long-running component at all. See
`ARCHITECTURE.md` §2–3 for the full comparison.
